...
def revenue_per_region(dp: DataProcessor) -> Dict:
    ...
    """
    ######################################## YOUR CODE HERE ##################################################
    data_reader =  dp.data_reader #### [YOUR CODE HERE] ####
    data_reader_gen = (row for row in data_reader) #### [YOUR CODE HERE] ####
    ...
    for row in tqdm(data_reader_gen):
        if row[constants.OutDataColNames.COUNTRY] not in aggregate:
            aggregate[row[constants.OutDataColNames.COUNTRY]] = dp.to_float(row['TotalPrice']) #### [YOUR CODE HERE] ####
        aggregate[row[constants.OutDataColNames.COUNTRY]] += dp.to_float(row['TotalPrice']) #### [YOUR CODE HERE] ####
